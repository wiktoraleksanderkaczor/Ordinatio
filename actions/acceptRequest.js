						...
						if(request.type === "shift") {
								dbController.storeShift(request.employeeId, request.dateStart, request.dateEnd, request.timeStart, request.timeEnd, function (err, result) { 
									if(err) {
										...
										dbController.storeMessage(req.user.id, request.employeeId, moment().format("YYYY-MM-DD - HH:mm"), "Your shift request (request id: " + request.requestId + ") was approved.", function(err, result) {
											if(err) {
												console.log(err);
											} 
											else {
												...
												dbController.storeMessage(req.user.id, req.user.id, moment().format("YYYY-MM-DD - HH:mm"), "You approved shift request #" + request.requestId + " for employee #"+ request.employeeId + " - " + request.firstName + " " + request.surname + " (" + request.username + ")", function (err, result) {
													if(err) {
														console.log(err);
													}
													else {
							...
							dbController.storeHoliday(request.employeeId, request.dateStart, request.dateEnd, function (err, result) { 
								...
								dbController.storeMessage(req.user.id, request.employeeId, moment().format("YYYY-MM-DD - HH:mm"), "Your holiday request (request id: " + request.requestId + ") was approved.", function (err, result) {
									if(err) {
										console.log(err);
									}
									else {
										...
										dbController.storeMessage(req.user.id, req.user.id, moment().format("YYYY-MM-DD - HH:mm"), "You approved holiday request #" + request.requestId + " for employee #" + request.employeeId + " - " + request.firstName + " " + request.surname + " (" + request.username + ")", function (err, result) {
											if(err) {
												console.log(err);
											}
											else {
